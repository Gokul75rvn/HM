...
async function seedDepartments() {
  ...
  return records.reduce<Record<string, string>>((acc, department) => {
    acc[department.code] = department.id;
    return acc;
  }, {});
}

async function seedUsers(passwordHash: string, departmentIds: Record<string, string>) {
  const users = [
    {
...
async function seedClinicalData(doctorId: string, patientId: string, departmentIds: Record<string, string>) {
